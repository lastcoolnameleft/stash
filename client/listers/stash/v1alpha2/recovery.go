...
type RecoveryLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.Recovery, err error)
...
// List lists all Recoveries in the indexer.
func (s *recoveryLister) List(selector labels.Selector) (ret []*v1alpha2.Recovery, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.Recovery))
	})
	return ret, err
}
...
type RecoveryNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.Recovery, error)
...
// List lists all Recoveries in the indexer for a given namespace.
func (s recoveryNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.Recovery, err error) {
...
// Get retrieves the Recovery from the indexer for a given namespace and name.
func (s recoveryNamespaceLister) Get(name string) (*v1alpha2.Recovery, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("recovery"), name)
	}
	return obj.(*v1alpha2.Recovery), nil
}
